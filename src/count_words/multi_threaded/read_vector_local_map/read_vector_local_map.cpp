#include "read_vector_local_map.h"
...
void read_vector_local_map(const std::vector<std::string> &list_files,
  ...
  std::generate_n(std::back_inserter(workers), n_threads, [&]() {
    return std::thread{task1,       std::ref(words), std::ref(maps[count++]),
